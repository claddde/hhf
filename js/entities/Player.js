...
export class Player extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y, texture = 'idle', animKey = null) {
    super(scene, x, y, texture);
    scene.add.existing(this);
    scene.physics.add.existing(this);

    // Animation key prefix: null = original HoodLust (unprefixed 'idle'/'walk'…);
    // otherwise '<id>' plays '<id>-idle' etc. for the extra characters.
  ...
  // Resolve an animation key for this character ('<prefix>-<state>' or bare).
  _animName(s) { return this.animKey ? `${this.animKey}-${s}` : s; }
  _hasAnim(s) { return this.scene.anims.exists(this._animName(s)); }

  _setState(s) {
    this.state = s;
    if (this._hasAnim(s)) this.play(this._animName(s), true);
    else if (this._hasAnim('idle')) this.play(this._animName('idle'), true); // fallback to idle pose
  }

  /** One-shot hurt flash + animation (returns to state machine after). */
  playHurt() {
    if (this._locked || !this._hasAnim('hurt')) return;
    const key = this._animName('hurt');
    this.play(key, true);
    this.state = '_hurt';
    this.once('animationcomplete-' + key, () => { if (!this._locked) this._setState('idle'); });
  }
  ...
  playDeath() {
    ...
    if (this._hasAnim('death')) this.play(this._animName('death'), true);
  }
  ...
  _animateFallback(dt, speed, maxSpeed) {
    if (this._hasAnim(this.state)) return; // real anims take over
    if (speed > 4) {
