...
export class Menu {
  ...
  constructor(game, joystick) {
    ...
    // Menu is interactive immediately — assets finish loading silently in the
    // background. Play is never blocked by a loading screen.
    this.ready = true;
    window.addEventListener('hoodlust-ready', () => {
      this.ready = true;
      if (this.el.status) this.el.status.textContent = 'Ready. Press Play.';
      if (this._pendingPlay) { this._pendingPlay = false; this.play(); }
    });
  ...
  play() {
    // If assets haven't finished the initial parse yet, queue and auto-start
    // the instant they do — no loading screen, no blocking.
    if (!this.game.scene.getScene('GameScene') || !this.game.textures.exists('idle')) {
      this._pendingPlay = true;
      if (this.el.status) this.el.status.textContent = 'Starting…';
      return;
    }
  ...
  travelTo(regionId) {
    if (!this.game.textures.exists('idle')) { this._pendingPlay = false; }
    ...
    const web3 = window.HOODLUST.ui.web3;
    if (web3.isVerified() || web3.session.demo) go();
