...
export class BootScene extends Phaser.Scene {
  ...
  preload() {
    // Moderate parallelism: fast enough, but avoids the connection pile-up
    // that could stall the loader on slow/static hosts.
    this.load.maxParallelDownloads = 12;

    // Robust loading: a single missing/failed asset must NEVER freeze the
    // loading screen. Missing files are logged and skipped so `complete`
    // still fires, and a hard safety-timeout force-starts the game if the
    // loader ever hangs (e.g. a request stuck open on a flaky host).
    this._failed = [];
    this.load.on('loaderror', (file) => { this._failed.push(file && file.key); });
    this._safety = setTimeout(() => {
      if (this.scene.isActive('BootScene') && !this._done) {
        console.warn('[Boot] load safety-timeout — starting anyway', this._failed);
        try { this.load.removeAllListeners(); } catch (_) {}
        this._finish();
      }
    }, 20000);
  ...
  create() {
    ...
    if (this._failed && this._failed.length) console.warn('[Boot] skipped missing assets:', this._failed);
    this._finish();
  }
  ...
  _finish() {
    ...
    if (this.textures.exists('player')) this.textures.get('player').setFilter(Phaser.Textures.FilterMode.NEAREST);
    ...
    const A = (key, frames, frameRate, repeat) => {
      if (!this.textures.exists(key)) return;   // skip if the sheet failed to load
    ...
    // Signal the DOM menu that the engine is ready → enables Play.
