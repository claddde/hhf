...
import { PROGRESSION_ORDER, REGIONS, MAPS } from './data/maps.js';
...
const worldState = new WorldState(save);
const sfx = new SFX();
...
// Phase 2/3/4/5/7.5 in-game UI singletons (created once; reused across restarts).
const ui = { hud: new HUD(), dialogs: new Dialogs(), windows: new Windows(save), select: new SelectScreen(save), web3: new Web3UI(save), eco: new EcosystemUI(save), worldmap: new WorldMapUI(save, worldState) };

// World Map fast-travel → launch/switch the run into the chosen region.
ui.worldmap._onTravelDefault = (regionId) => menu.travelTo(regionId);

// Expose for debugging / future phases.
window.HOODLUST = { game, menu, joystick, ui, save, sfx, ambientAudio, worldState, CONFIG, __maps: { PROGRESSION_ORDER, REGIONS, MAPS } };
